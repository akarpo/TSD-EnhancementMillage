...
p=doc.add_paragraph();p.paragraph_format.space_after=Pt(2)
sf(p.add_run(nd("Troy’s own millage email names special education as the problem, then proposes six broad categories "
   "that never mention it. If it passes, dedicate it to special education, with measurable results.")),
   12,False,True,GRAY)
...
para(doc,"On June 30, 2026, the Troy School District emailed residents a newsletter urging a “yes” vote on the "
 "1.5-mill county enhancement millage. Read closely, it points to what the money should do. In the same email, the "
 "District (1) names the real funding problem, special education; then (2) proposes to spend the money on six broad "
 "budget categories, none of which mentions it; and (3) leans on a “44% of Michigan students already have this” pitch "
 "that implies Oakland is behind, when the state’s own data show its local funding is well ahead. If special education "
 "is the need, the reasonable step is to point the money at it and report the results.")
...
# SECTION 1
heading(doc,"1.  The need they named, special education, is real and countable")
para(doc,"The District is right that the gap is real, though its own “40%” figure needs care. When Congress passed the "
 "Individuals with Disabilities Education Act (IDEA), it authorized federal funding of up to 40% of the extra cost of "
 "educating students with disabilities: a ceiling Congress allowed itself, not a promise it made. In FY2025 the federal "
 "share was about 10.2% of average per-pupil spending. Michigan’s often-quoted “28.6%” is not its coverage rate either; "
 "it is a constitutional floor fixed by the Michigan Supreme Court in Durant (1997). The Michigan House Fiscal Agency puts "
 "actual coverage of approved special-education costs at 86%, leaving a shortfall of $616.8 million a year (FY2023-24). "
 "That is a real number, published by the Legislature’s own fiscal agency, and more useful than a borrowed percentage. "
...
# SECTION 2
heading(doc,"2.  Handed the money, not yet pointed at the need")
para(doc,"Having named the problem, the email does not yet commit a dollar to solving it. Instead it notes that the "
 ...
 "in 2031, whether the $57 million Troy will have collected actually did what was promised. Several of the six are things "
 "a district can genuinely measure, which is exactly why setting a target, committing the dollars, and reporting the "
 "results would be straightforward. That step simply has not been taken yet.")
...
# SECTION 4
heading(doc,"A better proposal: fund the need they named",color=GREEN)
para(doc,"This is not an argument that schools cannot use money. It is an argument for spending it where the District itself "
...
para(doc,"For Troy alone, the millage is roughly $9.6 million a year, about $57 million over six years. Aimed squarely at "
 "special education, that is enough to make a real, measurable dent in the gap the District describes. Spread thinly across "
 "general operations, it would leave nothing specific to point to.",after=8)
...
para(doc,"To its credit, the District’s email does disclose the cost: about $150 a year on a $200,000 home, $300 on a $400,000 "
 "home, and about $326 on a home at Troy’s 2025 median sale price of roughly $435,000 (about $1,958 over six years), levied "
 "on your primary residence, unlike the 18-mill operating tax. So this is not about hiding the price, and it is not a knock "
 "on the District, which makes a fair case that schools are stretched. It is a simple ask. The District points to federal "
 "special-education underfunding as the squeeze on its budget. If this passes, the resources to relieve that squeeze are in "
 "hand. Fund the need you named, and show the results.")
...
para(doc,"• District communication: Troy School District, “Oakland County Enhancement Millage Proposal,” email newsletter, "
 ...
 "Macomb ISD enhancement (1.8198 mills levied 2025, 2020-2029) per Macomb ISD; MISD’s audited financials report "
 "$66,430,519 levied for the year ended 6/30/2024, and project over $70M for 2024-25. Population: U.S. Census 2024.\n"
 "• Special-education funding: IDEA’s 40% maximum authorized share and the FY2025 federal share of about 10.2% of "
 "national average per-pupil expenditure, per Congressional Research Service R44624. Michigan’s 86% actual coverage, the "
 "$616.8M shortfall (FY2023-24), and the 28.6138% Durant constitutional floor, per Michigan House Fiscal Agency, “Fiscal "
 "Brief: Special Education Funding” (Feb. 2026). Context: Michigan League for Public Policy; Autism Alliance of Michigan.\n"
...
para(doc,"Facebook posts are plain text (no tables), so the numbers go in as images. To post: paste the caption below, then "
 "click the photo icon and attach BOTH images from your Downloads folder in this order: (1) problem-vs-plan.png, then "
 "(2) funding-chart.png. The caption points to them as the “first image” and “the chart.” Both images are shown at the bottom of "
 "this page so you can confirm which is which.",9.5,False,False,GRAY,after=8)
fb=(
"📖 Read the Troy School District’s own email about the Aug. 4 enhancement millage. It points to what the money should do. "
...
"“unrestricted”… “a variety of uses.” Not one dollar is yet committed to the special-education gap it just named.\n\n"
...
"If it passes, spend it where they said the need is, and report the results. 🗳️  #Aug4  #OaklandCounty"
)
nd(fb)
...
sf(p.add_run(nd("Attach #1 (the “first image”):  problem-vs-plan.png")),10.5,True,False,NAVY)
doc.add_picture(f"{SCR}/problem-vs-plan.png",width=Inches(6.4));doc.paragraphs[-1].alignment=WD_ALIGN_PARAGRAPH.CENTER
p=doc.add_paragraph();p.paragraph_format.space_before=Pt(8)
sf(p.add_run(nd("Attach #2 (“the chart”):  funding-chart.png")),10.5,True,False,NAVY)
doc.add_picture(f"{SCR}/funding-chart.png",width=Inches(6.4));doc.paragraphs[-1].alignment=WD_ALIGN_PARAGRAPH.CENTER
...
out=f"{SCR}/TSD-Enhancement-Millage-Analysis.docx"
